        ...
        */
        ...
        void executeWithIotaValues(std::vector<Key_t>& keys, std::vector<Value_t>& values, std::vector<Offset_t>& offsets){
            ...
            thrust::for_each(
                ...
                [&](Offset_t index){
                    const BucketSize upperLimit = std::min(BucketSize(maxValuesPerKey), std::numeric_limits<BucketSize>::max());
                    
                    const std::size_t begin = offsets[index];
                    const std::size_t end = offsets[index+1];
                    const std::size_t num = end - begin;
                    const std::size_t newNum = std::min(num, static_cast<std::size_t>(upperLimit));

                    if(newNum < num){
                        //remove values which exceed bucket size
                        valuesPerKey_begin[index] = newNum;
                    
                        for(std::size_t k = begin + newNum; k < end; k++){
                            removeflags[k] = true;
                        }
                    }
                }
        ...
        void executeWithIotaValues(std::vector<Key_t>& keys, std::vector<Value_t>& values, std::vector<Offset_t>& offsets){
            ...
            thrust::for_each(
                ...
                [=] __device__ (std::size_t index){
                    const BucketSize upperLimit = std::min(BucketSize(maxValuesPerKey_copy), std::numeric_limits<BucketSize>::max());
                    ...
                    if(newNum < num){
                        //remove values which exceed bucket size
                        d_valuesPerKey_begin[index] = newNum;
        ...
        struct WarpcoreTransformer{
            ...
            static Result execute(
                ...
                for(std::size_t i = 0; i < iters; i++){
                    ...
                    std::copy_n(keys.begin() + begin, num, h_keys_array[which].begin());
                    CUDACHECK(cudaMemcpyAsync(d_keys_array[which].data(), h_keys_array[which].data(), sizeof(Key) * num, H2D, stream_array[which]));
                    care::gpu::fixKeysForGpuHashTable<Key, Value>(d_keys_array[which].data(), num, stream_array[which]);
                    std::copy_n(values.begin() + begin, num, h_values_array[which].begin());
                    CUDACHECK(cudaMemcpyAsync(d_values_array[which].data(), h_values_array[which].data(), sizeof(Value) * num, H2D, stream_array[which]));
