...
/*
    Removes elements from sorted range which occur less than k times.
    If a range of equal elements is greater than or equal to k, only the first element of this range is kept.
    Returns end of the new range.
    The new range is empty if there are more than max_num_elements unique elements
*/

template<class InputIter, class OutputIter, class Equal>
OutputIter remove_by_count_unique_with_limit(
    OutputIter output,
    InputIter first,
    InputIter last,
    std::size_t k,
    ...
    if(std::distance(first, last) == 0) return output;
    if(elements_to_copy > k) return output;

    OutputIter copytobegin = output;
    InputIter copyfrombegin = first;
    InputIter copyfromend = first;
    ++copyfromend;
...
OutputIter remove_by_count_unique_with_limit(
    ...
    using T = typename std::iterator_traits<InputIter>::value_type;
    return remove_by_count_unique_with_limit(output, first, last, k, max_num_elements, std::equal_to<T>{});
}
