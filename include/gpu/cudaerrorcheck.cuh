...
#define CUDACHECKIMPL(ans, async, abort) { \
    using namespace std::string_literals; \
    cudaError_t MY_CUDA_CHECK_status = (ans);                                 \
    if (MY_CUDA_CHECK_status != cudaSuccess){              \
        cudaGetLastError();                 \
...
#define CUDACHECK(ans) { CUDACHECKIMPL((ans), false, true); }

#define CUDACHECKASYNC { CUDACHECKIMPL((cudaPeekAtLastError()), true, true); }
