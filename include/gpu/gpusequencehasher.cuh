...
namespace gpusequencehasher{
    ...
    void minhashSignatures3264Kernel(
        ...
        for(int index = tid; index < numSequences * numHashFuncs; index += stride){
            const int mySequenceIndex = index / numHashFuncs;
            const int myNumHashFunc = index % numHashFuncs;
...
struct GPUSequenceHasher{
    ...
    Result hash(
        ...
        // dim3 block(128,1,1);
        // const int numBlocks = SDIV(numHashFuncs * numSequences, block.x);
        // dim3 grid(std::min(65535, numBlocks),1,1);

        auto kernel = gpusequencehasher::minhashSignatures3264Kernel<HashValueType>;
        ...
        const int numBlocks = SDIV(numHashFuncs * numSequences, blocksize);
