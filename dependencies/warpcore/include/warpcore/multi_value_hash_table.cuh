...
public:
    ...
    using self_type = MultiValueHashTable<
        Key,
        Value,
        EmptyKey,
        TombstoneKey,
    ...
    */
        ...
        const cudaStream_t stream = 0) const noexcept
    {
        ...
        if(keys_out != nullptr)
        {
            ...
            auto functor = [=] DEVICEQUALIFIER (const key_type& key)
            {
                helpers::atomicAggInc(tmp);
            };

            auto foreachkernel = kernels::for_each_unique_key_gridstrided<decltype(functor), self_type>;

            constexpr int blocksize = MAXBLOCKSIZE;
            int deviceId = 0;
            int numSMs = 0;
            ...
            const int numBlocks = SDIV(capacity()*cg_size(), blocksize);
            dim3 grid(std::min(numBlocks, maxBlocks), 1, 1);

            foreachkernel<<<grid, block, 0, stream>>>
            (functor, *this);
...
private:
    ...
    template<class Func, class Core>
    GLOBALQUALIFIER
    friend void kernels::for_each_unique_key_gridstrided(Func, const Core);
